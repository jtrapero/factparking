...
// Datos de clientes simulados (en una app real vendrían de una BBDD)
const clients = [
  {
    id: "1",
...
export default function NuevaFacturaPage() {
  const [invoiceData, setInvoiceData] = useState({
    clientId: "",
    invoiceDate: new Date().toISOString().split("T")[0], // Fecha actual por defecto
    invoiceNumber: "", // Se cargará dinámicamente
    observations: "",
    paymentType: "TRANSFERENCIA",
    paymentEntity: "LA CAIXA",
    paymentCcc: "ES51 2100 1549 6102 0029 2896",
    otherObservations: "80 DE MESES ANTERIORES",
    billingPeriod: "", // Nuevo campo para el mes de facturación
  })
  const [selectedClient, setSelectedClient] = useState<(typeof clients)[0] | null>(null)
  const [items, setItems] = useState<InvoiceItem[]>([
    { id: 1, description: "ABONO PLAZA PARKING ENERO", price: 240.0, quantity: 1, total: 240.0 },
  ])
  ...
  useEffect(() => {
    // Cargar el siguiente número de factura al montar el componente
    const fetchNextInvoiceNumber = async () => {
      try {
        const response = await fetch("/api/invoices/next-number")
        const data = await response.json()
        setInvoiceData((prev) => ({ ...prev, invoiceNumber: data.nextInvoiceNumber }))
      } catch (error) {
        console.error("Error al obtener el siguiente número de factura:", error)
        setInvoiceData((prev) => ({ ...prev, invoiceNumber: "ERROR" })) // Manejo de error
      }
    }
    fetchNextInvoiceNumber()

    // Establecer el período de facturación por defecto al mes actual
    const today = new Date()
    const monthNames = [
      "Enero",
      "Febrero",
    ...
    setInvoiceData((prev) => ({
      ...prev,
      billingPeriod: `${monthNames[today.getMonth()]} ${today.getFullYear()}`,
    }))
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Aquí iría la lógica para guardar la factura en la BBDD
    alert("Factura guardada con éxito (simulado)!")
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Crea una nueva factura para un cliente</p>
        </div>
